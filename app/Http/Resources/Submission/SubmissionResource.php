...
class SubmissionResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'blanks' => $this->resource->relationLoaded('blanks', $this->resource->blanks),
            'principal' => $this->resource->relationLoaded('principal', function () {
                return [
                    'id' => $this->resource->principal->id,
                    'name' => $this->resource->principal->name,
                ];
            }),
            'guarantor' => $this->resource->relationLoaded('guarantor', function () {
                return [
                    'id' => $this->resource->guarantor->id,
                    'name' => $this->resource->guarantor->name,
                ];
            }),
            'guarantor_branch' => $this->resource->relationLoaded('guarantorBranch', function () {
                return [
                    'id' => $this->resource->guarantorBranch->id,
                    'name' => $this->resource->guarantorBranch->name,
                ];
            }),
            'product' => $this->resource->relationLoaded('product', function () {
                return [
                    'id' => $this->resource->product->id,
                    'name' => $this->resource->product->name,
                ];
            }),
            'product_type' => $this->resource->relationLoaded('guarantorToProductType', function () {
                return [
                    'id' => $this->resource->guarantorToProductType->id,
                    'name' => $this->resource->guarantorToProductType->name,
                ];
            }),
            'obligee' => $this->resource->relationLoaded('obligee', function () {
                return [
                    'id' => $this->resource->obligee->id,
                    'name' => $this->resource->obligee->name,
                ];
            }),
            'staff' => $this->resource->relationLoaded('staff', function () {
                return [
            ...
            'submission_before' => $this->resource->relationLoaded('submissionBefore', function () {
                return [
