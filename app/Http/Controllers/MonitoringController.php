...
class MonitoringController extends Controller
{
    ...
    public function submissionDetail(Submission $submission): Response
    {
        ...
        $contractValueFormatted = $this->formatCurrency($submission->contract_value);
        $guaranteeValueFormatted = $this->formatCurrency($submission->guarantee_value);

        $requiredDocs = RequiredDoc::query()
            ->get(['id', 'product_type_id', 'name', 'description', 'created_at'])
            ->map(function ($doc) use ($principalDocs) {
                $principalDoc = $principalDocs->firstWhere('required_doc_id', $doc->id);
                $doc->setAttribute('name', $principalDoc?->name ?? $doc->name);
                $doc->setAttribute('url', $principalDoc?->url ? Storage::url($principalDoc->url) : null);

                return $doc->only(['id', 'name', 'description', 'url']);
            });

        $firstYearRatio = $submission->first_year_ratio;
        $lastYearRatio = $submission->last_year_ratio;
        $ratios = $principal->principalRatios;

        if ($firstYearRatio && $lastYearRatio) {
            $ratios = $ratios->whereIn('year', [$firstYearRatio, $lastYearRatio]);
        }

        $principal->setAttribute('ratios', $ratios->values()->toArray());

        $submission->scores->each(function ($score) {
            $score->setAttribute('category_name', $score->scoringQuestionCategory->name ?? '-');
            $score->setAttribute('question_name', $score->scoringQuestion->name ?? '-');
            $score->setAttribute('option_name', $score->scoringOption->name ?? '-');
        });
        ...
        $startDate = Carbon::parse($submission->start_date)->translatedFormat('d F Y');
        $endDate = Carbon::parse($submission->end_date)->translatedFormat('d F Y');

        $documentFormats = DocumentFormat::query()
            ->whereNull(['guarantor_id', 'product_id', 'guarantor_to_product_type_id'])
            ->orWhere(function ($query) use ($submission) {
                $query->where('guarantor_id', $submission->guarantor_id)
                    ->where('product_id', $submission->product_id)
                    ->where(function ($query) use ($submission) {
                        $query->where('guarantor_to_product_type_id', $submission->guarantor_to_product_type_id)
                            ->orWhereNull('guarantor_to_product_type_id');
                    });
            })
            ->orderBy('no')
        ...
        $submissionDocsFile = $submission->submissionDocs->whereNotNull('url')->values();
        $submissionDocs = $submission->has_send_to_guarantor
          ? $submission->submissionDocs->whereNotNull('document_format_id')->values()
          : collect();

        $submissionDocs = $submissionDocs->sortBy(fn ($doc) => $doc->documentFormat?->no)->values();
        ...
        if ($callback) {
            $callback->setAttribute('url', $callback->url ? Storage::url($callback->url) : null);
        }

        $submissionInheritId = $submission->submission_inherit_id;
        $employeeLimit = $submission->employeeLimit
            ?->firstWhere('employee_id', auth()->id())
            ?->getAttribute($submissionInheritId ? 'limit_inherit' : 'limit', 0);

        $beyondTheLimit = $employeeLimit < $submission->guarantee_value;

        $supportDocs = $submission->supportDocs->map(function ($doc) {
            $doc->setAttribute('name', $doc->name ?? '-');
            $doc->setAttribute('number', $doc->number ?? '-');
            $doc->setAttribute('date', $doc->date ? Carbon::parse($doc->date)->translatedFormat('d F Y') : null);
            if ($doc->url) {
                $doc->setAttribute('url', Storage::url($doc->url));
            }

            return $doc;
        });

        $blankId = $submission->blank_id;
        $blanks = Blank::query()
            ->where('profile_id', $profileId)
            ->where(function ($query) use ($blankId) {
                $query->where('id', $blankId)->orWhere('is_picked', false);
            })->get();

        $totalScore = $submission->scores->sum('point');
