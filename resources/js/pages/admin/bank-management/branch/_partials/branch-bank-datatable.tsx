...
interface BranchBankDatatableProps {
  banks: any;
}

const BranchBankDatatable: React.FC<BranchBankDatatableProps> = ({ banks }) => {
  const deleteBank = (bank: any) => {
    router.delete(route(BranchBankUtils.link.destroy, bank.id));
  };
          ...
          {banks?.data?.length > 0 ? (
            banks?.data?.map((bank: any, index: number) => (
              <TableRow key={bank.id}>
                ...
                <TableCell>{bank.email ?? "Email Belum Dimasukan"}</TableCell>
                    ...
                    <DropdownMenuContent className="w-36 mr-8 mt-1">
                      ...
                      <DropdownMenuItem asChild className="cursor-pointer">
                        <Link
                          ...
                          href={route(BranchBankUtils.link.edit, bank.id)}>
                          Edit
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
...
export default BranchBankDatatable;
