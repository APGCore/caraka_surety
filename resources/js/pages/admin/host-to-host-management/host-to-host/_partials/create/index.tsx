...
export default function HostToHostCreate() {
  const { data, setData, post } = useForm();
  const [open, setOpen] = useState<boolean>(false);

  const handleSubmit = () => {
    post(route(HostToHostUtils.link.store), {
      preserveState: true,
      preserveScroll: true,
      onError: (errors) => {
        console.log("Gagal menambahkan data host to host", errors);
      },
      onSuccess: () => {
        console.log("Berhasil menambahkan data host to host");
        setOpen(false);
      },
    });
  };
