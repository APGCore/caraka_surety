...
const SubmissionDetailPage: SubmissionDetailPageProps = ({ submission }) => {
    ...
    const replaceHasilAnalisaPlaceholders = (template: string, data: any) => {
        return template
            ...
            .replace(
                "[UNDERLYING]",
                data?.contract_doc_name + " " + data?.contract_doc_number + " " + data?.job_name || "",
            )
            ...
            <Show when={submission?.beyond_the_limit}>
                <div className="fixed top-20 w-[81%] z-[100]">
                    <Alert variant="destructive">
                        ...
                        <AlertDescription>
                            Nilai Jaminan {submission.guarantee_value_formatted} Pengajuan Melebihi Batas Kewenangan
                            yaitu Rp. {textCurrency(submission.limit)}.
                        </AlertDescription>
                    </Alert>
                </div>
            </Show>
            <main className={"space-y-10 w-[800px] mx-auto mt-[50px]"}>
                ...
                <div className="border rounded-sm p-4 space-y-6 bg-white">
                    ...
                    <div>
                        <Alert
                            variant={
                                isProcess ? "warning" : isApproved ? "success" : isRejected ? "destructive" : "default"
                            }>
                                ...
                                {(() => {
                                    ...
                                    if (submission?.document_format_guarantor?.length) {
                                        const filteredGuarantorDocs = submission.document_format_guarantor.filter(
                                            (doc: any) =>
                                                doc.product_id === null && doc.guarantor_to_product_type_id === null,
                                        );
