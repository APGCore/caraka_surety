...
const NewFileInput: React.FC<InputFileProps> = ({
  className,
  limit,
  onFileChange,
  reset,
  ...
  const KBtoMB = (size: number) => (size / 1024).toFixed(2);
      ...
      <button
        ...
        {!files ? (
          <>
            ...
            {limit && (
              <span className="text-xs text-gray-500">Ukuran file tidak boleh lebih dari {KBtoMB(limit)} MB</span>
            )}
          </>
        ) : (
